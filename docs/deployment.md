...
│  │              Nginx (:80 / :443)                  │  │
│  │                                                 │  │
│  │  your-other-app.example.com/api/*   → :8000           │  │
│  │  your-domain.example.com/api/* → :8002           │  │
│  └──────────────────────┬──────────────────────────┘  │
│                         │                             │
│  ┌──────────────┐  ┌───┴──────────┐                  │
...
```
用户访问 your-domain.example.com
        │
        ├── 页面请求 → Vercel CDN（前端静态资源）
        │
        └── /api/* 请求 → DNS A记录 → 阿里云 Nginx
                              │
                              ├── Host: your-domain.example.com → :8002
                              └── Host: your-other-app.example.com   → :8000
```
...
# 进入已有的 PostgreSQL 容器
docker exec -it <postgres_container> psql -U globaleads
...
```env
# 数据库（共用 PG，不同数据库）
DATABASE_URL=postgresql+asyncpg://globaleads:your_password@localhost:5432/globaleads
...
```bash
# 备份 globaleads 数据库
docker exec <postgres_container> pg_dump -U globaleads globaleads > globaleads_backup_$(date +%Y%m%d).sql
```

建议设置 crontab 定期备份：
```bash
# 每天凌晨 3 点备份
0 3 * * * docker exec <postgres_container> pg_dump -U globaleads globaleads > /opt/globaleads/backups/globaleads_$(date +\%Y\%m\%d).sql
```
...
server {
    listen 80;
    server_name your-other-app.example.com;
...
# 申请证书
sudo certbot --nginx -d your-other-app.example.com -d your-domain.example.com
